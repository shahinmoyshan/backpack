        ...
        <div class="flex flex-col md:flex-row gap-3 md:items-center mt-8 py-2">
            ...
            <?php if (in_array('save', $submit_buttons)): ?>
                <button type="button" x-on:click="$refs.breadFormAction.name = '_create', $refs.breadFormSubmit.click()"
                    class="px-4 py-2.5 text-sm font-medium tracking-wide text-white transition-colors duration-300 transform bg-accent-600 shadow-sm shadow-accent-200 rounded-lg hover:bg-accent-500 focus:outline-hidden focus:ring-3 focus:ring-accent-300/80"><?= _e(__('create')) ?></button>
            <?php endif ?>
            <?php if (in_array('save_and_continue', $submit_buttons)): ?>
                <button type="button" x-on:click="$refs.breadFormAction.name = '_create_add', $refs.breadFormSubmit.click()"
                    class="px-4 py-2.5 text-sm font-medium tracking-wide text-white transition-colors duration-300 transform bg-primary-700 shadow-sm shadow-primary-200 rounded-lg hover:bg-primary-800 focus:outline-hidden focus:ring-3 focus:ring-primary-300/80"><?= _e(__('create & create another')) ?></button>
            <?php endif ?>
