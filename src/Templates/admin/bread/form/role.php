<div class="bg-white shadow sm:rounded-lg px-6 py-2 mb-8">
    <h3 class="text-xl mb-2 border-b py-3"><?= __e('General') ?></h3>
...
if (!($form->hasField('id') && intval($form->getValue('id', 0)) === user('admin_roles_id'))):
    ...
    ?>
    <div class="bg-white shadow sm:rounded-lg px-6 py-2" permissiongroup x-init="$el.querySelectorAll('button[permission-all]').forEach(b => b.addEventListener('click', (event) => {
    ...
    if (group) {
        ...
        <?php if (!empty($permissions)): ?>
            <div class="flex mb-2 border-b py-3 items-center justify-between">
                <h3 class="text-xl"><?= __e('Permissions') ?></h3>
                <div class="flex items-center gap-2">
                    <button permission-all="allow" type="button"
                        class="text-sm px-2.5 py-1 font-medium hover:bg-zinc-300 rounded-sm border border-zinc-400/75 bg-zinc-200 text-zinc-800"><?= __e('Allow All') ?></button>
                    <button permission-all="deny" type="button"
                        class="text-sm px-2.5 py-1 font-medium hover:bg-zinc-300 rounded-sm border border-zinc-400/75 bg-zinc-200 text-zinc-800"><?= __e('Deny All') ?></button>
                </div>
            </div>
            <?php foreach ($permissions as $group => $permission):
                ...
                <div class="mb-4" permissiongroup>
                    <?php if (count($permission) > 1): ?>
                        <div class="mb-2 border-b py-2 flex items-center justify-between">
                            <h3 class="font-medium"><?= _e(ucwords($group)) ?></h3>
                            <div class="flex justify-end items-center gap-2">
                                <button permission-all="allow" type="button"
                                    class="text-[0.8rem] px-1.5 py-0.5 font-medium hover:bg-zinc-300 rounded-sm border border-zinc-400/75 bg-zinc-200 text-zinc-800"><?= __e('Allow All') ?></button>
                                <button permission-all="deny" type="button"
                                    class="text-[0.8rem] px-1.5 py-0.5 font-medium hover:bg-zinc-300 rounded-sm border border-zinc-400/75 bg-zinc-200 text-zinc-800"><?= __e('Deny All') ?></button>
                            </div>
                        </div>
                    <?php endif ?>
